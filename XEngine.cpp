...
DirextXEng::DirextXEng(HINSTANCE hInstance)
    : D3DApp(hInstance)
{
...
void DirextXEng::CreateRtvAndDsvDescriptorHeaps()
{
...
void DirextXEng::UpdateObjectCBs(const GameTimer& gt)
{
...
void DirextXEng::UpdateSkinnedCBs(const GameTimer& gt)
{
...
void DirextXEng::UpdateMaterialBuffer(const GameTimer& gt)
{
...
void DirextXEng::UpdateShadowTransform(const GameTimer& gt)
{
...
void DirextXEng::UpdateShadowPassCB(const GameTimer& gt)
{
...
void DirextXEng::UpdateSsaoCB(const GameTimer& gt)
{
...
void DirextXEng::BuildSsaoRootSignature()
{
    CD3DX12_DESCRIPTOR_RANGE texTable0;
...
void DirextXEng::BuildDescriptorHeaps()
{
	//
	// Create the SRV heap.
	//
...
void DirextXEng::BuildShadersAndInputLayout()
{
...
void DirextXEng::BuildShapeGeometry()
{
...
void DirextXEng::LoadSkinnedModel()
{
	auto geo = SkinnedMeshLoader::LoadSkinnedModel(
		mSkinnedModelFilename,    // Имя файла модели
		md3dDevice.Get(),             // Устройство D3D12
		mCommandList.Get(),           // Командный список D3D12
		mSkinnedInfo,                  // Информация о скелетной модели
		mSkinnedModelInst,             // Указатель на экземпляр модели
		mSkinnedSubsets,               // Подсети
		mSkinnedMats                   // Материалы
	);

	mGeometries[geo->Name] = std::move(geo);
}

void DirextXEng::BuildPSOs()
{
...
void DirextXEng::BuildFrameResources()
{
...
void DirextXEng::BuildMaterials()
{
...
void DirextXEng::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
...
void DirextXEng::DrawSceneToShadowMap()
{
...
void DirextXEng::DrawNormalsAndDepth()
{
	mCommandList->RSSetViewports(1, &mScreenViewport);
    mCommandList->RSSetScissorRects(1, &mScissorRect);
...
CD3DX12_CPU_DESCRIPTOR_HANDLE DirextXEng::GetCpuSrv(int index)const
{
...
CD3DX12_GPU_DESCRIPTOR_HANDLE DirextXEng::GetGpuSrv(int index)const
{
...
CD3DX12_CPU_DESCRIPTOR_HANDLE DirextXEng::GetDsv(int index)const
{
...
CD3DX12_CPU_DESCRIPTOR_HANDLE DirextXEng::GetRtv(int index)const
{
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 7> DirextXEng::GetStaticSamplers()
{
